...
static std::string autoindex_page(const std::string& fs_dir, const std::string& uri) {
    ...
    for (const std::string& name : names) {
        ...
        page << "<a href=\"" << esc << "\">" << esc << "</a>" << std::string(pad, ' ') << mtime
             << "  " << size << "\n";
    }
...
static void serve_file(int client_socket, const char* file_path, const char* request_path,
                       const char* request, const std::string& set_cookie_header, bool head_only) {
    ...
    if (stat(resolved_path, &path_stat) == 0 && S_ISDIR(path_stat.st_mode)) {
        ...
        if (!index_found) {
            ...
            log_request_response(request, response_header);
            if (send(client_socket, response_header.c_str(), response_header.length(),
                     DVWS_SEND_FLAGS) < 0) {
    ...
    const std::string last_modified = have_stat ? http::http_date(file_stat.st_mtime) : "";
    ...
    if (have_stat) {
        response_header +=
            "Content-Length: " + std::to_string((long long)file_stat.st_size) + "\r\n";
        response_header += "Last-Modified: " + last_modified + "\r\n";
    }
    if (!set_cookie_header.empty()) response_header += set_cookie_header;
    response_header += std::string("Server: ") + http::kServerBanner + "\r\n";
...
void serve_static(int client_socket, const HttpRequest& req, const std::string& set_cookie_header) {
    ...
    if (joined.rfind(docroot, 0) != 0) {
        http::send_status(
            client_socket, "403 Forbidden", "text/html; charset=utf-8",
            http::error_page(403, "Forbidden", "The requested path is outside the document root."),
    ...
    if (ec) {
        http::send_status(
            client_socket, "404 Not Found", "text/html; charset=utf-8",
            http::error_page(404, "Not Found", "The requested path could not be resolved."),
            set_cookie_header, req.method == "HEAD");
        return;
    }
    // weakly_canonical() drops a trailing separator; restore it.
    if (!joined.empty() && !file_path.empty() && joined.back() == '/' && file_path.back() != '/') {
        file_path += "/";
    }
    ...
    if (dir_len > 0 && file_path[dir_len - 1] != '/' && stat(file_path.c_str(), &path_stat) == 0 &&
        ...
        if (redirectable) {
            ...
            http::send_status(client_socket, "301 Moved Permanently", "text/html; charset=utf-8",
                              http::error_page(301, "Moved Permanently", "The document has moved."),
                              extra, head_only);
            return;
        }
    }

    serve_file(client_socket, file_path.c_str(), req.clean_path, req.raw, set_cookie_header,
               head_only);
}
